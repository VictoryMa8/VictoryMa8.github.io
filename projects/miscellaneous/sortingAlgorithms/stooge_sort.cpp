...
void stooge_sort_helper(int arr[], int start, int stop) {
    ...
    int one_third = start + (stop - start + 1) / 3;
    int two_thirds = stop - (stop - start + 1) / 3;
...
void timing_experiment(void (*func)(int*, int)) {
    // open output file
    ofstream my_file;
    my_file.open("stooge_sort_runtimes.csv");
    my_file << "n, time\n";

    for (int i = 0; i <= 600; i += 10) { // create arrays with 0, 10, 20, ..., 1000 size
        int* test = new int[i]; // array with size i
        long long total_time = 0; // total time for one size
        for (int trials = 0; trials <= 50; trials++) { // test the current size 1000 times
