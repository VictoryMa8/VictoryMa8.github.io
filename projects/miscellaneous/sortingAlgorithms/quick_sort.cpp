...
void quick_sort_helper(int arr[], int start, int stop) {
    if (stop <= start) {
        return;
    }
    ...
    quick_sort_helper(arr, start, i - 2);
    quick_sort_helper(arr, i, stop);
}
...
void timing_experiment(void (*func)(int*, int)) {
    // open output file
    ofstream my_file;
    my_file.open("quick_sort_runtimes.csv");
    my_file << "n, time\n";
